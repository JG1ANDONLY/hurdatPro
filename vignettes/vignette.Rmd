...
From the plot, we observe that the number of landfalling hurricanes displays an
increasing trend over years. Thus, we are interested in building a time series 
regression model.
...
# Subset the data to include only hurricanes that made landfall in the US
landfall_hurricanes <- new_dat[new_dat$landfall, ]

# Aggregate the data to count the number of landfalling hurricanes per year
landfall_counts <- aggregate(id ~ year, data = landfall_hurricanes, FUN = function(x) length(unique(x)))

# Fit a Poisson regression model to the landfall counts
model <- glm(id ~ year, data = landfall_counts, family = poisson)

# Extract the estimated parameters and standard errors
estimates <- summary(model)$coefficients[, 1:2]

# Calculate the hypothesis test p-value for beta1
pvalue <- summary(model)$coefficients[2, 4]

# Calculate the 95% confidence intervals for beta0 and beta1
confint <- confint.default(model)[2:3,]
...
After conducting some research, we were able to confirm that Hurricane Katrina in 2005 made landfall with maximum accumulated cyclone energy on the Mississippi-Alabama border. This finding is consistent with the results obtained from our analysis in Analysis Question 2, where we visualized the accumulated cyclone energy for hurricanes in the Gulf of Mexico region.

APA citation: National Weather Service. (n.d.). Hurricane Katrina. Retrieved from https://www.weather.gov/mob/katrina#:~:text=Known%20for%20its%20storm%20surge,Rosa%20Sound%20in%20northwest%20Florida.
